...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(
        ...
        // Demo mode: on demo.nlmanager.cc all write operations are blocked for everyone
        Gate::before(function (User $user, string $ability) {
            if (!app()->runningInConsole() && request()->getHost() === 'demo.nlmanager.cc') {
                $readOnly = ['view', 'viewAny', 'viewApiDocs'];
                foreach ($readOnly as $prefix) {
                    if ($ability === $prefix || str_starts_with($ability, $prefix)) {
                        return null; // allow reads
                    }
                }
